...
class sfp_dnsresolve(SpiderFootPlugin):
    ...
    # Option descriptions
    optdescs = {
        'skipcommononwildcard': "如果检测到通配符DNS，只尝试从普通子域列表中查找第一个普通子域",
        'validatereverse': "在考虑将其作为目标的别名之前，要验证反向解析的主机名是否仍能解析回该IP",
    ...
    def setup(self, sfc, userOpts=dict()):
        ...
        self.parentresults = self.tempStorage()
    ...
    def enrichTarget(self, target):
        ...
        if target.targetType == 'NETBLOCK_OWNER':
            return target
    ...
    def watchedEvents(self):
        return [
            ...
            "AFFILIATE_INTERNET_NAME", "IP_ADDRESS", "IPV6_ADDRESS",
            "INTERNET_NAME", "AFFILIATE_IPADDRESS", "DOMAIN_NAME",
            # Events that may contain hostnames in their content
            "DOMAN_WHOIS", "NETBLOCK_WHOIS", "AFFILIATE_DOMAIN_NAME"
            "RAW_DNS_RECORDS", "RAW_RIR_DATA", "SSL_CERTIFICATE_RAW",
            "SSL_CERTIFICATE_ISSUED", "TCP_PORT_OPEN_BANNER", "WEBSERVER_BANNER"
        ]
    ...
    def producedEvents(self):
        return ["IP_ADDRESS", "INTERNET_NAME", "AFFILIATE_INTERNET_NAME",
                "AFFILIATE_IPADDRESS", "DOMAIN_NAME", "IPV6_ADDRESS",
                "DOMAIN_NAME_PARENT", "AFFILIATE_DOMAIN_NAME"]
    ...
    def handleEvent(self, event):
        ...
        if eventName not in ["CO_HOSTED_SITE", "AFFILIATE_INTERNET_NAME",
                             ...
                             "INTERNET_NAME", "AFFILIATE_IPADDRESS"]:
        ...
        if eventName in ["IP_ADDRESS", "INTERNET_NAME", "IPV6_ADDRESS", "AFFILIATE_DOMAIN_NAME",
                         "AFFILIATE_IPADDRESS", "AFFILIATE_INTERNET_NAME", "DOMAIN_NAME"]:
    ...
    def processHost(self, host, parentEvent, affiliate=None):
        ...
        if htype in ["INTERNET_NAME", "DOMAIN_NAME"]:
            dom = self.sf.hostDomain(host, self.opts['_internettlds'])
            if not dom:
                return None
        ...
        if htype in ["AFFILIATE_INTERNET_NAME", "AFFILIATE_DOMIAN_NAME"]:
            dom = self.sf.hostDomain(host, self.opts['_internettlds'])
            if not dom:
                return None
    ...
    def processDomain(self, domainName, parentEvent, affil=False, host=None):
        ...
        if self.getTarget().matches(domainName):
            domevt = SpiderFootEvent("DOMAIN_NAME", domainName,
                                     self.__name__, parentEvent)
            self.notifyListeners(domevt)
            if domainName not in self.parentresults:
                domevt = SpiderFootEvent("DOMAIN_NAME_PARENT", domainName,
                                     self.__name__, parentEvent)
                self.parentresults[domevt.data] = True
                self.notifyListeners(domevt)
        else:
