...
class sfp_dnsraw(SpiderFootPlugin):
    ...
    def producedEvents(self):
        return ["DNS_MX", "DNS_NS", "RAW_DNS_RECORDS", "DNS_TXT", "DNS_SPF",
            ...
            for x in res.answer:
                ...
                for rx in list(recs.keys()):
                    ...
                    for m in grps:
                        ...
                        if rx == "MX":
                            evt = SpiderFootEvent("DNS_MX", strdata.lower(), self.__name__, parentEvent)
                            self.notifyListeners(evt)
                            domains.append(strdata.lower())

                        if rx == "NS":
                            evt = SpiderFootEvent("DNX_NS", strdata.lower(), self.__name__, parentEvent)
                            self.notifyListeners(evt)
                            domains.append(strdata.lower())

                        if rx == "TXT":
                            evt = SpiderFootEvent("DNS_TXT", strdata, self.__name__, parentEvent)
                            self.notifyListeners(evt)
