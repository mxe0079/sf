...
class sfp_grep_app(SpiderFootPlugin):
    ...
    meta = {
        'name': "grep.app",
        'summary': "搜索grep.app API，寻找与指定域名相关的链接和电子邮件",
        'flags': [""],
        ...
        'dataSource': {
            ...
            'description': "grep.app从GitHub上的50多万个公共存储库中搜索代码",
        }
    }
    ...
    # Option descriptions
    optdescs = {
        'max_pages': "要取的结果的最大页数",
        'dns_resolve': "DNS解析每个确定的域"
    }
    ...
    def producedEvents(self):
        return ["EMAILADDR", "EMAILADDR_GENERIC", "DOMAIN_NAME",
                "INTERNET_NAME", "RAW_RIR_DATA", "LINKED_URL"]
            ...
            for result in data:
                ...
                if links:
                    for link in links:
                        ...
                        evt = SpiderFootEvent('LINKED_URL', link, self.__name__, event)
                        self.notifyListeners(evt)
