...
class sfp_censys(SpiderFootPlugin):
    ...
    meta = {
        ...
        'dataSource': {
            ...
            'apiKeyInstructions': [
                "访问 https://censys.io/",
                "注册免费账号",
                "访问链接 https://censys.io/account",
                "点击 'API'",
            ],
            'favIcon': "https://censys.io/assets/favicon.png",
            'logo': "https://censys.io/assets/logo.png",
            'description': "发现攻击者的暴露和其他常见入口点, Censys不断扫描整个互联网，包括不知名的端口"
        }
    }
    ...
    optdescs = {
        ...
        'delay': '请求之间的延迟，以秒为单位',
        "age_limit_days": "忽略任何超过这个天数的记录。0 = 无限"
    }
        ...
        for addr in qrylist:
            ...
            try:
                ...
                if 'headers' in rec:
                    ...
                    e = SpiderFootEvent("WEBSERVER_BANNER", dat, self.__name__, pevent)
                    e.actualSource = addr
                    self.notifyListeners(e)
